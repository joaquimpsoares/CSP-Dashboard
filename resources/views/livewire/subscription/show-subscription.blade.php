<div>
    {{-- @dd($subscription->products->where('instance_id',$subscription->instance_id)->first()->upgrade_target_offers)
    {{-- @foreach ($subscription->products->where('instance_id',$subscription->instance_id)->first()->getUpgradeProducts()->all() as $upgrade)
    @dd($upgrade)

    @foreach ($upgrade as $upgrade1)
    ...
    <tr>
        ...
        <td>
            <div class="w-16 pt-0 mb-3">
                <x-input class="relative w-full px-2 py-1 text-sm " type="number" name="amount_addon" value="{{$item->amount}}"></x-input>
            </div>
        </td>
        <td>{{$item->unitType}}</td>
        <td></td>
...
<div>
    <form wire:submit.prevent="save({{$subscription->id}})">
        <x-modal.slideout wire:model.defer="showEditModal">
            <x-slot name="title">Edit Reseller</x-slot>
            <x-slot name="content">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="form-group">
                                            <select wire:model="editing.status_id" name="status" class="form-control @error('editing.status') is-invalid @enderror" sf-validate="required">
                                                <option value="{{$subscription->status->id}}" selected>{{ucwords(trans_choice($subscription->status->name, 1))}}</option>
