<div @if($subscription->status_id == 1)  wire:init="getSubscription({{ $subscription->id }})" @endif>
    <div class="mx-auto md:max-w-5xl pt-14">
        ...
        <x-bladewind.alert class="rounded-md">
            Your subscription is expiring on the  {{date('j F, Y', strtotime($subscription->expiration_data))}}.
            <a wire:click="edit({{ $subscription->id }})" href="#">Renew now</a>
        </x-bladewind.alert>
        @endif
        @if($subscription->status_id == 2)
        <x-bladewind.alert class="rounded-md" type="error">
            Your subscription is {{ ucwords(trans_choice($status, 1)) }}.
        </x-bladewind.alert>
        @endif
        ...
        @endif
        @if($subscription->status_id == 4)
        <x-bladewind.alert class="rounded-md" type="error">
            Your subscription is {{ ucwords(trans_choice($status, 1)) }}.
        </x-bladewind.alert>
        @endif
        <div @if(!$subscription->productonce->IsNCE() && !$subscription->product->IsPerpetual()) wire:init="validateisEligible({{ $subscription->id }})" @endif>
            <div class="relative z-0 flex-col flex-1 overflow-y-auto">
                <div class="p-4 overflow-hidden bg-white">
                    <div class="lg:flex lg:items-center lg:justify-between">
                        <div class="flex-1 min-w-0">
                            <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">{{$subscription->name}}
                                <span wire:loading>
                        ...
                        <div class="flex justify-between mt-5 lg:mt-0 lg:ml-4">
                            <span class="sm:ml-15">
                                <div x-data="{ open: false }"   @keydown.escape.stop="open = false; focusButton()" @click.away="onClickAway($event)" class="relative inline-block ml-20">
                                    ...
                                    <div  x-cloak x-show.transition="open" @click.away="open = false" class="absolute right-0 z-10 w-40 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                        @if($status != 'messages.canceled' && $status != 'messages.inactive' && !$subscription->product->IsPerpetual())
                                        <div class="py-1" role="none">
                                            <a wire:click="edit({{ $subscription->id }})" href="#" class="block w-full px-4 py-2 text-sm text-left hover:bg-gray-50 disabled:text-gray-500" role="menuitem" tabindex="-1" id="menu-item-4">
                                        ...
                                        <div class="py-1" role="none">
                                            ...
                                            <button wire:click="$toggle('showconfirmationModal')" class="block w-full px-4 py-2 text-sm text-left text-yellow-500 hover:bg-gray-50 disabled:text-gray-500" role="menuitem" tabindex="-1" id="menu-item-6">
                                            ...
                                            <button wire:click="enable({{$subscription->id}})" class="block w-full px-4 py-2 text-sm text-left text-green-700 hover:bg-gray-50 disabled:text-gray-500" role="menuitem" tabindex="-1" id="menu-item-6">
                                            ...
                                            @if($status == 'messages.active')
                                            <button wire:click="$toggle('showcancelconfirmationModal', 'checkCancelationWindow')" class="block w-full px-4 py-2 text-sm text-left text-red-700 hover:bg-gray-50 disabled:text-gray-500" role="menuitem" tabindex="-1" id="menu-item-6">
                ...
                @endif
                @endif
                <!-- start subscription details card -->
                <div class="mb-4 overflow-hidden bg-white shadow sm:rounded-lg">
                    <div class="px-4 py-3 sm:px-6">
                        <h3 class="text-lg font-medium leading-6 text-gray-900">{{ ucwords(trans_choice('messages.subscription_details', 1)) }}</h3>
                    </div>
                    <div class="px-4 py-3 border-t border-gray-200 sm:px-6">
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                            <div class="sm:col-span-1">
                                <dt class="text-sm font-medium text-gray-500">{{ ucwords(trans_choice('messages.customer', 1)) }}</dt>
                                <dd class="mt-1 text-sm text-gray-900"><a href="{{$subscription->customer->format()['path']}}" >
                            ...
                            <div class="sm:col-span-1">
                                ...
                                <dd class="mt-1 text-sm text-gray-900">{{$subscription->created_at->format('j F, Y')}}</dd>
                            </div>
                            <div class="sm:col-span-1">
                                <dt class="text-sm font-medium text-gray-500">{{ ucwords(trans_choice('messages.billing_cycle', 1)) }}</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{strtoupper($subscription->billing_period)}}</dd>
                            </div>
                            <div class="sm:col-span-1">
                                <dt class="text-sm font-medium text-gray-500">{{ ucwords(trans_choice('messages.product_term', 1)) }}</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{strtoupper($subscription->term)}}</dd>
                            </div>
                            <div class="sm:col-span-1">
                ...
                <!-- Ends subscription details card -->


                <!-- start price table card -->
                <div class="mb-4 overflow-hidden bg-white shadow sm:rounded-lg">
                    <div class="px-4 py-3 sm:px-6">
                        <h3 class="text-lg font-medium leading-6 text-gray-900">{{ ucwords(trans_choice('messages.product', 1)) }}</h3>
                    </div>
                    <div class="px-4 py-3 border-t border-gray-200 sm:px-6">
                ...
                <!-- Ends price table card -->

                <!-- start order details card -->
                <div class="mb-4 overflow-hidden bg-white shadow sm:rounded-lg">
                    <div class="px-4 py-3 sm:px-6">
                        <h3 class="text-lg font-medium leading-6 text-gray-900">{{ ucwords(trans_choice('messages.order', 2)) }}</h3>
                    </div>
                    <div class="px-4 py-3 border-t border-gray-200 sm:px-6">
                        <div class="ml-6">
                            <div class="mt-4 mb-8"><ol class="relative border-l border-gray-200 dark:border-gray-700">
                                ...
                                <li class="mb-10 ml-6">
                                    ...
                                    <h3 class="flex items-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">{{$subscription->name}}
                                    ...
                                    <time class="block mb-2 text-xs font-normal leading-none text-gray-400 dark:text-gray-500">{{$item->created_at}}</time>
                                    <p class="mb-2 text-sm text-base font-normal text-gray-500 dark:text-gray-400">{{$item->details}}</p>
                                </li>
                                @endforeach
                                @endif
                                ...
                                <li class="mb-10 ml-6">
                                    ...
                                    <h3 class="flex items-center mb-1 text-sm font-medium font-semibold text-gray-900 dark:text-white">
                                        {{$subscription->name}}
                                        <span class="bg-blue-100 text-blue-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-3">
                                            {{ ucwords(trans_choice($item->status->name, 1)) }}
                                        </span>
                        ...
                        <a wire:click="loadOrders" class="font-medium text-indigo-600 no-underline bg-transparent cursor-pointer hover:text-gray-900 hover:no-underline focus:shadow-xs" href="#">
