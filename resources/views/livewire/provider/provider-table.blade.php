<div>
            ...
            <div class="flex flex-col">
                <div class="flex flex-col items-center justify-between lg:flex-row">
                    <div class="flex items-center">
                        <h4 class="text-base font-semibold text-slate-900">{{ ucwords(trans_choice('messages.provider_table', 2)) }}</h4>
                    </div>
                                    ...
                                    <div x-data="{
                                        ...
                                        <template x-teleport="body">
                                            <div x-cloak x-show="open" @click.away="open = false" @scroll.window="open = false" @resize.window="open = false"
                                                ...
                                                <button type="button" @click="callEdit({{ $provider->id }}); open = false"
                                                ...
                                                @canImpersonate
                                                    @if(!empty($provider->format()['mainUser']))
                                                        <a class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
                                                           href="{{ route('impersonate', $provider->format()['mainUser']['id'])}}">
    ...
    <div>
        @if($showEditModal == true)
            <form wire:submit.prevent="submit" wire:key="provider-edit-form-{{ $editingId ?? 'new' }}">
                <x-modal.slideout wire:model="showEditModal" :closeOnBackdrop="false">
                    ...
                    @elseif($showCreateUser == true)
                        <x-slot name="title">{{ ucwords(trans_choice('messages.create', 1)) }} {{ ucwords(trans_choice('messages.provider', 1)) }}</x-slot>
                    @endif
                        ...
                        <section class="dark-grey-text">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="mb-3">
                                        <label class="block text-sm font-medium text-slate-700" for="address_2">
                                            {{ ucwords(trans_choice('messages.address_2', 1)) }} (optional)
                                        </label>
                                        <input
                            ...
                            @if ($showCreateUser == true)
                                <div class="mt-8 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                                    <h3 class="text-sm font-semibold text-slate-900">{{ ucwords(trans_choice('messages.user_information', 1)) }}</h3>
                                    <p class="mt-0.5 text-xs text-slate-600">{{ ucwords(trans_choice('messages.create', 1)) }} {{ ucwords(trans_choice('messages.user', 1)) }} (Default status: Active)</p>
                                </div>
                                ...
                                <div class="row mt-4">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <x-input wire:model.defer="creatingUserForm.name" type="text" class="@error('creatingUserForm.name') is-invalid @enderror" id="name" name="name" placeholder="First Name" value="{{ $creatingUserForm['name'] ?? '' }}"></x-input>
                                            @error('creatingUserForm.name')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
                                        </div>
                                        <div class="form-group">
                                            <x-label for="last_name">@lang('Last Name')</x-label>
                                            <x-input wire:model.defer="creatingUserForm.last_name" type="text" class="@error('creatingUserForm.last_name') is-invalid @enderror" id="last_name" name="last_name" placeholder="Last Name" value="{{ $creatingUserForm['last_name'] ?? '' }}"></x-input>
                                            @error('creatingUserForm.last_name')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                <x-input wire:model.defer="creatingUserForm.socialite_id" class="@error('creatingUserForm.socialite_id') is-invalid @enderror" type="text" name="socialite_id" id='socialite_id' value="{{ $creatingUserForm['socialite_id'] ?? '' }}"></x-input>
                                                @error('creatingUserForm.socialite_id')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <x-label for="phone">@lang('Phone')</x-label>
                                            <x-input wire:model.defer="creatingUserForm.phone" type="text" class="@error('creatingUserForm.phone') is-invalid @enderror" id="phone_number" name="phone" placeholder="Phone" value="{{ $creatingUserForm['phone'] ?? '' }}"></x-input>
                                            @error('creatingUserForm.phone')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
                                        </div>
                                        <div class="form-group">
                                            <x-label for="address">@lang('Address')</x-label>
                                            <x-input wire:model.defer="creatingUserForm.address" type="text" class="@error('creatingUserForm.address') is-invalid @enderror" id="address" name="address" placeholder="Address" value="{{ $creatingUserForm['address'] ?? '' }}"></x-input>
                                            @error('creatingUserForm.address')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
                                        </div>
                                    </div>
                                </div>
