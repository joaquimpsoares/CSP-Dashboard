        ...
        <div class="mx-auto md:max-w-5xl pt-14">
            <div class="relative z-0 flex-col flex-1 overflow-y-auto">
                <div class="p-4 overflow-hidden bg-white">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-col mt-1 mb-5 sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6">
                                <div class="flex items-center mt-2 text-sm text-gray-500">
                                    <div class="flex items-center mt-2 text-sm text-gray-500">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $provider->status->name == 'messages.active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'  }}  capitalize">
                                ...
                                <div class="flex items-center mt-2 text-sm text-gray-500">
                                    <div class="flex items-center mt-2 text-sm text-gray-500">
                                        ...
                                        {{ ucwords(trans_choice('messages.created_at', 2)) }} {{$provider->created_at->format('j F, Y')}}
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div x-data="{ open: false }" @keydown.escape.stop="open = false; focusButton()" class="relative inline-block px-3 mt-6 text-left">
                                    ...
                                    <div x-cloak x-show.transition="open" @click.away="open = false" class="absolute right-0 z-10 w-auto mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                        <div class="py-1" role="none">
                                            <a href="#" wire:click="edit({{ $provider->id }})" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="menu-item-4">
                                                <x-icon.edit></x-icon.edit>
                                                {{ ucwords(trans_choice('messages.edit', 1)) }}
                                            </a>
                                        </div>
                                        <div class="py-1" role="none">
                                            <a href="#" wire:click="ImportSubscriptions({{ $provider->id }})"  wire:loading.attr="disabled" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="menu-item-4">
                                        ...
                                        <div class="py-1" role="none">
                                            ...
                                            <a href="{{ route('impersonate', $provider->format()['mainUser']['id']) }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="menu-item-4">
                                        ...
                                        <div class="py-1" role="none">
                                            @if($provider->status->name == 'messages.active')
                                            ...
                                            @endif
                                            @if($provider->status->name != 'messages.active')
                                            <a href="#" wire:click="enable({{ $provider->id }})" class="block px-4 py-2 text-sm text-green-700" role="menuitem" tabindex="-1" id="menu-item-6">
                    ...
                    <!-- start provider details card -->
                    <div class="mb-4 overflow-hidden bg-white shadow sm:rounded-lg">
                        <div class="px-4 py-3 sm:px-6">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ ucwords(trans_choice('messages.provider_details', 1)) }}</h3>
                        </div>
                ...
                <div class="mb-4 overflow-hidden bg-white shadow sm:rounded-lg">
                    <div class="px-4 py-3 sm:px-6">
                        <h3 class="text-lg font-medium leading-6 text-gray-900">{{ ucwords(trans_choice('messages.reseller', 2)) }}</h3>
                    </div>
                    <div class="px-4 py-3 border-t border-gray-200 sm:px-6">
                            ...
                            <div class="mt-8 -mx-4 overflow-hidden ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 ">
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($provider->resellers as $key => $subscription)
                                        {{-- <tr>
