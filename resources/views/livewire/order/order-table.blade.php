                ...
                <div class="flex flex-col">
                    ...
                    <x-tableazure>
                        <x-slot name="head">
                            ...
                            {{-- <x-table.heading sortable multi-column visibility='hidden' tablecell='lg:table-cell' wire:click="sortBy('country_id')"       :direction="$sorts['country_id'] ?? null">{{ ucwords(trans_choice('messages.details', 1)) }}</x-table.heading>
                            <x-table.heading sortable multi-column visibility='hidden' tablecell='lg:table-cell' wire:click="sortBy('country_id')"       :direction="$sorts['country_id'] ?? null">{{ ucwords(trans_choice('messages.cost', 2)) }}</x-table.heading> --}}
                            {{-- <x-table.heading sortable multi-column visibility='hidden' tablecell='lg:table-cell' wire:click="sortBy('country_id')"       :direction="$sorts['country_id'] ?? null">{{ ucwords(trans_choice('messages.status', 2)) }}</x-table.heading>
                            <x-table.heading sortable multi-column visibility='hidden' tablecell='lg:table-cell' wire:click="sortBy('country_id')"       :direction="$sorts['country_id'] ?? null">{{ ucwords(trans_choice('messages.created_at', 2)) }}</x-table.heading>
                            <x-table.heading sortable multi-column visibility='hidden' tablecell='lg:table-cell' wire:click="sortBy('country_id')"       :direction="$sorts['country_id'] ?? null">{{ ucwords(trans_choice('messages.updated_at', 2)) }}</x-table.heading> --}}
                        </x-slot>
                        <x-slot name="body">
                            ...
                            <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $value['id'] }}">
                                ...
                                <x-table.cell>
                                    <a href="#" wire:click="show({{ $value['id'] }})" class="w-full h-full p-0 m-0 no-underline bg-transparent border-0 cursor-pointer hover:text-gray-900 hover:no-underline">
                                        <div class="h-full py-2 pl-1 pr-2 m-0 overflow-auto">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium leading-4 capitalize">
                                                {{-- {{ number_format(($value['orderproducts']['quantity']*$value['orderproducts']['retail_price']) * ($value['orderproducts']['billing_cycle'] === 'annual' ? 12 : 1 ),2) }} --}}
                                            </span>
                                        </div>
                                    </a>
                                </x-table.cell>
                            ...
                            <div class="space-y-24">
                                <div class="text-sm sm:grid-rows-1 sm:grid-cols-12 sm:gap-x-6 md:gap-x-8 lg:gap-x-8">
                                    <div class="mt-6 sm:col-span-7 sm:mt-0 md:row-end-1">
                                        ...
                                        <p class="mt-1 font-medium text-gray-900">{{ $value->orderproduct->quantity }}</p>

                                        <p class="mt-1 font-medium text-gray-900">{{ number_format(($value->orderproduct->quantity*$value->orderproduct['retail_price']) * ($value->orderproduct['billing_cycle'] === 'annual' ? 12 : 1 ),2) }}</p>
                                        <h3 class="text-lg font-medium text-gray-900">Order Details</h3>
                                        <textarea name="" id="" cols="70" rows="5">{{$order->details}}</textarea>
                                    </div>
                                    <div class="sm:col-span-12 md:col-span-7">
                                        <dl class="grid grid-cols-1 py-8 border-b border-gray-200 gap-y-8 sm:grid-cols-2 sm:gap-x-6 sm:py-6 md:py-10">
                                            ...
                                            <div>
                                                <dt class="font-medium text-gray-900">User Details</dt>
                                                <dd class="mt-3 text-gray-500">
