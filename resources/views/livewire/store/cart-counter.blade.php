...
<div>
    ...
    <div class="flex items-center px-6 py-2 -mx-8 hover:bg-gray-100">
        ...
        <div>
            <select wire:model="cycle" class="block w-full p-2 text-sm text-gray-600">
                <option value="" selected="selected" hidden>Choose one</option>
                @foreach($item->billing_cycle as $cycle)
                <option wire:click="removeItem('{{ $item->id }}')"  value="{{ $cycle }}" >
                    {{ucfirst($cycle) }}
                </option>
                @endforeach
            </select>
