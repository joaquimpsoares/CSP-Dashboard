                    ...
                    <div class="">
                        <dl>
                            ...
                            <div class="py-1 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                {{-- @if($reseller->microsoftTenantInfo->first())
                                    <dd class="text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        <button value="copy" onclick="copyToClipboard('copy_{{ $reseller->microsoftTenantInfo->first()->tenant_domain }}')" class="inline-flex p-0 -mt-1 -mb-px -ml-1 overflow-visible no-underline normal-case bg-transparent border-0 cursor-pointer focus:shadow-xs" type="button">
                                            <div class="relative flex flex-row-reverse items-baseline p-0 m-0">
                                                <div class="flex flex-row-reverse items-baseline justify-start flex-auto p-0 m-0">
                                                    ...
                                                    <span >
                                                        <input id="copy_{{ $reseller->microsoftTenantInfo->first()->tenant_domain }}" value="{{ strtoupper($reseller->microsoftTenantInfo->first()->tenant_domain) }}" class="inline w-48 mr-1 font-mono text-xs font-normal" />
                                                    </span>
                                                </div>
                                            </div>
                                        </button>
                    ...
                    <div class="flex justify-between mt-4 mb-8">
                        <div class="">
                            <dl>
                                <div class="py-1 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm font-medium text-gray-500">{{ ucwords(trans_choice('messages.company_name', 1)) }}</dt>
                                    <dd class="text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{$reseller->provider->first()->company_name}}</dd>
                                </div>
                                <div class="py-1 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm font-medium text-gray-500">{{ ucwords(trans_choice('messages.main_contact', 1)) }}</dt>
                                    <dd class="text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{$reseller->provider->first()->users->first()->name}} {{$reseller->provider->first()->users->first()->last_name}}</dd>
                                </div>
                                <div class="py-1 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm font-medium text-gray-500">{{ ucwords(trans_choice('messages.phone', 1)) }}</dt>
                                    <dd class="text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{$reseller->provider->first()->users->first()->phone}}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
                                        ...
                                        <td class="px-2 py-2 text-sm font-medium text-gray-900 whitespace-wrap lg:table-cell">
                                            <div class="p-0 mt-px mb-0 ml-px mr-0 pointer-events-auto">
                                                ...
                                                <td class="hidden px-2 py-2 text-sm font-medium text-gray-900 whitespace-nowrap lg:table-cell">
                                                    <span class="inline font-medium text-gray-900">
                                                        @php
                                                        $teste =$customer->subscriptions->map(function ($subscriptions, $key) use($customer) {
                                                            if (!$subscriptions->order->isEmpty()) {
                                                                $test = $subscriptions->order->map(function ($item, $key) use($subscriptions) {
                                                                    if ($item->orderproduct != null) {
                                                                        $sum = ($item->orderproduct->retail_price*$subscriptions->amount)*($subscriptions->billing_period === 'annual' ? 12 : 1 );
                                                                        return $sum;
                                                                    }
                                                               });
                                                                return $test->sum();
