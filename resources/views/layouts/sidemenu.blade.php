                    ...
                    <div x-show="open" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" x-description="Off-canvas menu, show/hide based on off-canvas menu state." class="relative flex flex-col flex-1 w-full max-w-xs pt-5 pb-4 bg-white" style="display: none;">
                        ...
                        <div x-data="{ open: false }"   @keydown.escape.stop="open = false; focusButton()" @click.away="onClickAway($event)" class="relative inline-block px-3 mt-6 text-left">
                            ...
                            <div x-cloak x-show.transition="open" @click.away="open = false" class="absolute top-0 right-0 z-40 w-48 py-2 mt-12 bg-white border border-gray-100 rounded-lg shadow-md">
                                ...
                                <a href="{{Route('profile.showprofile',Auth::user()->id)}}" class="block px-4 py-2 text-gray-600 hover:bg-gray-100 hover:text-blue-600">Account
                                    Settings
                                </a>
                                ...
                                {{-- <a href="#" onclick="event.preventDefault(); document.getElementById('gdpr-form').submit();" class="block px-4 py-2 text-gray-600 hover:bg-gray-100 hover:text-blue-600"> --}}
                                    {{-- <form id="gdpr-form" action="/gdpr/download" method="POST">
                                        ...
                                        @csrf
                                        <div class="col-md-6">
                                            <label for="password">{{ ucwords(trans_choice('messages.password', 1)) }}</label>
                                            <input name="password" type="text" class="form-control @error('password') is-invalid @enderror" id="password" placeholder="%" value="">
                        ...
                        <div class="flex-1 h-0 mt-6 overflow-y-auto">
                            <nav class="px-2">
                                <div class="space-y-1">
                                    <a href="{{route('home')}}" class="flex items-center w-full py-2 pl-2 pr-1 mt-1 text-sm font-medium text-gray-600 bg-white rounded-md hover:text-indigo-500 @if(request()->routeIs('home')) text-indigo-500 @endif focus:outline-none focus:ring-2 focus:ring-indigo-500 x-state:on="Current" x-state:off="Default" aria-controls="sub-menu-1" @click="open = !open" aria-expanded="true" x-bind:aria-expanded="open.toString()" x-state-description="Current: &quot;bg-gray-100 text-gray-900&quot;, Default: &quot;bg-white text-gray-600 hover:bg-gray-50 hover:text-gray-900&quot;">
