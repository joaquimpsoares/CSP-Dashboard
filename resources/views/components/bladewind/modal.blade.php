...
<div
    ...
    <div class="bg-white {{$sizes[$size]}} sm:{{$sizes[$size]}} mx-auto my-auto rounded-lg drop-shadow-2xl bw-{{$name}}">
    {{-- {{ $sizes[$size] }} --}}
        <div class="flex">
            @if($type !== '')
                <div class="py-6 pl-6 modal-icon">
                    <x-bladewind::modal-icon type="{{ $type }}"></x-bladewind::modal-icon>
                </div>
            @endif
            <div class="flex-grow p-6 modal-body">
